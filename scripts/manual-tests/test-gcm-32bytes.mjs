...
console.log('32字节明文:', Array.from(plaintext));
...
console.log('密文+MAC:', Array.from(ciphertext.subarray(0, encLen + finalLen)));
...
try {
  ...
  console.log('匹配:', decrypted.subarray(0, 32).every((v, i) => v === plaintext[i]));
} catch (e) {
  console.log('\n解密失败:', e.message);
}
