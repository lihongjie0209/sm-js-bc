# SM-JS-BC 示例代码

本目录包含 `sm-js-bc` 库的所有示例代码，与 README.md 中的代码示例一一对应。

## 📁 文件说明

| 文件 | 说明 | 对应 README 章节 |
|------|------|-----------------|
| `sm3-hash.mjs` | SM3 哈希计算示例 | SM3 哈希 |
| `sm2-keypair.mjs` | SM2 密钥对生成示例 | SM2 密钥对生成 |
| `sm2-sign.mjs` | SM2 数字签名示例 | SM2 数字签名 |
| `sm2-encrypt.mjs` | SM2 公钥加密示例 | SM2 公钥加密 |
| `sm2-keyexchange.mjs` | SM2 密钥交换示例 | SM2 密钥交换 |
| `sm4-ecb-simple.mjs` | SM4 ECB 模式简单示例 | SM4 对称加密 |
| `sm4-modes.mjs` | SM4 多种工作模式示例 | SM4 高级用法 |

## 🚀 快速开始

### 1. 安装依赖

```bash
cd example
npm install
```

这将安装父目录的 `sm-js-bc` 库（使用 `file:..` 依赖）。

### 2. 运行示例

#### 运行单个示例

```bash
# SM3 哈希
npm run sm3-hash

# SM2 密钥对生成
npm run sm2-keypair

# SM2 数字签名
npm run sm2-sign

# SM2 公钥加密
npm run sm2-encrypt

# SM2 密钥交换
npm run sm2-keyexchange

# SM4 ECB 模式简单示例
npm run sm4-ecb-simple

# SM4 多种工作模式示例
npm run sm4-modes
```

#### 或者直接使用 node 命令

```bash
node sm3-hash.mjs
node sm2-keypair.mjs
node sm4-ecb-simple.mjs
node sm4-modes.mjs
node sm2-sign.mjs
node sm2-encrypt.mjs
node sm2-keyexchange.mjs
```

#### 运行所有示例

```bash
npm run all
```

## 📝 示例说明

### SM3 哈希示例 (`sm3-hash.mjs`)

演示如何使用 `SM3Digest` 类计算数据的哈希值：
- 基本哈希计算
- 分段数据更新
- 空数据哈希

### SM2 密钥对生成示例 (`sm2-keypair.mjs`)

演示如何生成 SM2 密钥对：
- 生成单个密钥对
- 批量生成多个密钥对
- 查看密钥格式和长度

### SM2 数字签名示例 (`sm2-sign.mjs`)

演示 SM2 数字签名的完整流程：
- 生成密钥对
- 对消息进行签名
- 验证签名
- 篡改消息后验签（失败）
- 使用错误公钥验签（失败）
- 签名多条消息

### SM2 公钥加密示例 (`sm2-encrypt.mjs`)

演示 SM2 公钥加密/解密流程：
- 生成密钥对
- 使用公钥加密
- 使用私钥解密
- 加密不同长度的消息
- 处理 UTF-8 多字节字符
- 使用错误私钥解密（失败）

### SM2 密钥交换示例 (`sm2-keyexchange.mjs`)

演示 SM2 密钥交换协议（ECDH）：
- Alice 和 Bob 各自生成静态密钥对
- 生成临时密钥对
- 计算共享密钥
- 验证双方密钥一致
- 生成不同长度的共享密钥（128/192/256位）

### SM4 ECB 模式简单示例 (`sm4-ecb-simple.mjs`)

演示 SM4 对称加密基本用法（ECB 模式）：
- 生成随机 128 位密钥
- ECB 模式加密/解密（PKCS7 填充）
- 处理不同长度的数据（0-100 字节）
- 单块加密（16 字节，无填充）
- ⚠️ ECB 模式不安全，仅用于演示

### SM4 多种工作模式示例 (`sm4-modes.mjs`)

演示 SM4 的各种工作模式：
- ECB 模式（电子密码本）
- CBC 模式（密码块链接）
- CTR 模式（计数器模式）
- GCM 模式（认证加密）
- 使用底层 API 直接控制加密参数
- 对比不同模式的特性和安全性

## 🎯 预期输出

每个示例都会输出详细的执行过程和结果，例如：

```
=== SM3 哈希示例 ===

输入数据: Hello, SM3!
SM3 Hash: 9b8c9d5b8e7a6f5c4d3e2a1b0c9d8e7f6a5b4c3d2e1f0a9b8c7d6e5f4a3b2c1
哈希长度: 32 字节

--- 分段更新示例 ---
分段输入: "Hello, " + "SM3!"
SM3 Hash: 9b8c9d5b8e7a6f5c4d3e2a1b0c9d8e7f6a5b4c3d2e1f0a9b8c7d6e5f4a3b2c1
结果一致: true

--- 空数据哈希 ---
空数据 Hash: 1ab21d8355cfa17f8e61194831e81a8f22bec8c728fefb747ed035eb5082aa2b

✅ SM3 哈希示例运行完成
```

## 🔧 自定义示例

你可以基于这些示例创建自己的代码：

1. 复制任意示例文件
2. 修改代码以适应你的需求
3. 使用 `node your-example.mjs` 运行

## 📦 依赖说明

所有示例都只依赖打包后的 `sm-js-bc` 库，不依赖源代码：

```json
{
  "dependencies": {
    "sm-js-bc": "file:.."
  }
}
```

这意味着：
- ✅ 示例使用的是构建后的 `dist/` 文件
- ✅ 与最终用户的使用方式完全一致
- ✅ 可以验证打包后的库是否正常工作

## ⚠️ 注意事项

1. **确保已构建**：运行示例前，请先在父目录执行 `npm run build` 构建项目
2. **Node.js 版本**：需要 Node.js >= 20.0.0（ES2020 支持）
3. **重新构建**：如果修改了源代码，需要重新构建后示例才会生效

## 🐛 问题排查

### 错误：Cannot find package 'sm-js-bc'

**原因**：未安装依赖或父目录未构建

**解决**：
```bash
# 在父目录构建
cd ..
npm run build

# 在 example 目录安装
cd example
npm install
```

### 错误：Module not found

**原因**：Node.js 版本过低或模块导入路径错误

**解决**：
- 检查 Node.js 版本：`node --version`（需要 >= 20.0.0）
- 确保 package.json 中有 `"type": "module"`

## 📚 更多信息

- [主项目 README](../README.md)
- [API 文档](../docs/)
- [测试代码](../test/)

---

如有问题，欢迎提出 [Issue](https://github.com/lihongjie0209/sm-js-bc/issues)！
